...
export default function Overview({ items, categoryLabel, activeTab, onStart }: OverviewProps) {
  const sortedItems = [...items].sort((left, right) => {
    ...
    const rightMin = right.prices?.length
      ? Math.min(...right.prices.map((price) => price.p))
      : 9999;
    ...
    <>
      ...
      >
        ...
        <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#e8e6e1" }}>{categoryLabel}</div>
          ...
          {FUN_FACTS[categoryLabel] ? (
            ...
            >
              <span style={{ fontWeight: 700 }}>💡 Curiosidad · </span>
              {FUN_FACTS[categoryLabel]}
            </div>
          ) : null}
          <button
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
        {sortedItems.map((item, idx) => (
          <OverviewCard
            ...
            item={item}
            showCategoryLabel={activeTab === "Todo"}
            priority={idx < 2}
          />
        ))}
      </div>
