...
export default function StudyCard({ item, index, total, onPrimaryAction }: StudyCardProps) {
  ...
  const buttonLabel = hasRecipe ? "Memorizado — a recordar →" : "Memorizado — a practicar →";
      ...
      <div className="learn-card" style={{ position: "relative" }}>
        ...
        {ITEM_ORIGINS[item.name] ? (
          ...
          >
            {COUNTRY_FLAGS[ITEM_ORIGINS[item.name]] ? (
              <img
                src={`https://flagcdn.com/24x18/${COUNTRY_FLAGS[ITEM_ORIGINS[item.name]]}.png`}
                alt={ITEM_ORIGINS[item.name]}
                width={24}
                height={18}
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {item.method ? (
              <div style={{ fontSize: 12, color: "#8FC1E0", marginTop: 6 }}>{item.method}</div>
            ) : null}
            {item.garnish?.length ? (
