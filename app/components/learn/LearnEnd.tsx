...
export default function LearnEnd({
  totalReviewed,
  hadRecipes,
  recallStats,
  onRestart,
  ...
  const recallPct =
    recallStats.total > 0 ? Math.round((recallStats.hits / recallStats.total) * 100) : 0;
      ...
      <p style={{ color: "#9a9793", fontSize: 14 }}>
        ...
        {hadRecipes && recallStats.total > 0 ? (
          <>
            ...
            <br />
            {recallStats.hits}/{recallStats.total} ingredientes ({recallPct}%) ·{" "}
            {recallStats.perfect}/{recallStats.items} perfectos
          </>
        ) : null}
      </p>
