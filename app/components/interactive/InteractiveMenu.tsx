...
const SPIRIT_FAMILY_PATTERNS: Array<[RegExp, string]> = [
  ...
  [
    /whisky|whiskey|bourbon|jack daniels|tullamore|bulleit|monkey shoulder|macallan|bushmills|toki/i,
    "Whisky",
  ],
...
function getMainSpirit(item: MenuItem): string {
  ...
  const spiritCandidates = ingr.filter((ing) => {
    ...
    return (
      group === "spirit" &&
      !excludedSpirits.has(ing) &&
      (MENU_SPIRIT_HINTS.has(ing) || ordered.includes(ing))
    );
  });
...
function buildInteractiveMenuData(
  items: MenuItem[],
  preferCombinedPrice: boolean,
): InteractiveItem[] {
...
function matchesInteractiveFilters(item: InteractiveItem, state: InteractiveState): boolean {
  ...
  if (state.vinosVermuts.length && !state.vinosVermuts.some((v) => item.vinosVermuts.includes(v)))
    return false;
  if (state.bitters.length && !state.bitters.some((v) => item.bitters.includes(v))) return false;
  if (
    state.zumosRefrescos.length &&
    !state.zumosRefrescos.some((v) => item.zumosRefrescos.includes(v))
  )
    return false;
...
function getDisplayPrice(
  ...
  if (preferCombined) {
    const preferred =
      prices.find((entry) => entry.label === "combinado") ||
      prices.find((entry) => entry.label === "copa") ||
      prices[0];
    return { price: preferred.p, priceLabel: preferred.label };
  }

  const cheapest = prices.reduce(
    (lowest, entry) => (entry.p < lowest.p ? entry : lowest),
    prices[0],
  );
...
export default function InteractiveMenu() {
  ...
  const data = useMemo(
    () =>
      buildInteractiveMenuData(
        getInteractivePool(activeTab, activeFamily, false),
        preferCombinedPrice,
      ),
  ...
  const source = useMemo(
    () =>
      buildInteractiveMenuData(
        getInteractivePool(activeTab, activeFamily, true),
        preferCombinedPrice,
      ),
    [activeFamily, activeTab, preferCombinedPrice],
  );
  ...
  const list = useMemo(
    () => data.filter((item) => matchesInteractiveFilters(item, interactiveState)),
    [data, interactiveState],
  );

  const hasSubtabs =
    activeTab === AUTOR_CAT || activeTab === CLASICA_CAT || activeTab === COMBINADOS_TAB;
    ...
    <div className="interactive-shell">
      ...
      <div className="interactive-layout">
        <div className="interactive-filters">
          ...
          {available.bitters.length
            ? renderGroup("Bitters", available.bitters, "bitters", false)
            : null}
        ...
        <div className="interactive-cards">
          {list.length ? (
            ...
            <div className="interactive-empty">
              Ningún cóctel coincide con los filtros seleccionados.
            </div>
          )}
        </div>
      </div>
